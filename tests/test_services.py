...
Ручка `GET /api/services/` была ЕДИНСТВЕННОЙ в приложении без
`Depends(get_current_user)` — нашлось замером покрытия (S-31): она
не вызывалась в тестах ни разу, поэтому и отсутствие зависимости
никто не заметил. ПД она не отдаёт, но показывает состав интеграций
и то, настроен ли ключ OCR, то есть внутреннее состояние конфигурации.
"""
