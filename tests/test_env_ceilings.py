...
def test_сверка_молчит_когда_всё_сошлось(caplog):
    ...
    хорошее = {
        ...
        # Доверие заголовкам прокси — обязательная переменная с 21.09.2026
        # (S-67): без неё схема теряется МОЛЧА, поэтому в «норме» она есть.
...
# ─────────── доверие заголовкам прокси: S-67, вторая половина лечения ────────


def test_прибор_краснеет_когда_доверие_прокси_не_задано():
    """⚠️ ГЛАВНЫЙ СТОРОЖ ЭТОГО БЛОКА, И ОН ПРО МОЛЧАЛИВУЮ ПОЛОМКУ.

    Без `FORWARDED_ALLOW_IPS` uvicorn не верит `X-Forwarded-Proto` от Caddy,
    и редирект по слэшу уходит на `http://` — браузер блокирует его как
    Mixed Content. Дефект НЕ ВИДЕН НИОТКУДА: приложение отвечает, тесты
    зелены, в логах тихо. Он и провисел месяц, пока владелец не уперся
    в него руками 21.09.2026.

    Поэтому переменная попадает в опись со статусом «обязана быть»:
    единственный способ заметить её пропажу — прибор, который скажет
    об этом в журнал деплоя. Решение владельца 21.09.2026: «это не
    довесок, а вторая половина лечения».
    """
    from app import env_check

    имена = [имя for имя, *_ in env_check.ОЖИДАНИЯ]
    assert "FORWARDED_ALLOW_IPS" in имена, (
        "переменная выпала из описи — прибор перестал ловить молчаливую "
        "поломку схемы за прокси"
    )
    расхождения = dict((имя, угроза) for имя, _видно, угроза in env_check.сверить({}))
    assert "FORWARDED_ALLOW_IPS" in расхождения


def test_доверие_прокси_пустой_строкой_не_считается_заданным():
    """Пустая переменная в панели — частая форма «вроде поставил».

    Для uvicorn она равна отсутствию, значит и прибор обязан считать
    так же; иначе строка в панели успокаивала бы, ничего не включая.
    """
    from app import env_check

    for значение in ("", "   "):
        расхождения = [
            имя for имя, *_ in env_check.сверить({"FORWARDED_ALLOW_IPS": значение})
        ]
        assert "FORWARDED_ALLOW_IPS" in расхождения, значение


def test_заданное_доверие_прокси_расхождением_не_считается():
    """Парная проверка: с боевым значением прибор молчит.

    Без неё сторож выше был бы зелёным и на приборе, который ругается
    ВСЕГДА, — а такой прибор учит на себя не смотреть.
    """
    from app import env_check

    расхождения = [
        имя for имя, *_ in env_check.сверить({"FORWARDED_ALLOW_IPS": "172.16.0.0/12"})
    ]
    assert "FORWARDED_ALLOW_IPS" not in расхождения
