...
MUTATIONS = [
    ...
    (
        "M8 фото попадает в кэш промежуточных узлов (снят no-store)",
        RECEIPTS,
        '            media_type=content_type,\n            headers={"Cache-Control": "no-store"},',
        "            media_type=content_type,",
        "test_photo_key_is_proxied_and_signed_url_never_reaches_browser",
        PHOTO_TESTS,
    ),
    (
    ...
    # ── проксирование вместо редиректа и уборка снимков (S-48) ──
    (
        "M13 вернули редирект: подписанная ссылка уходит в браузер",
        RECEIPTS,
        "        return Response(\n            content=content,\n            media_type=content_type,",
        '        return RedirectResponse(\n            url=s3.presign_get_url(cfg, row["photo_key"]), status_code=302\n        )\n        return Response(\n            content=content,\n            media_type=content_type,',
        "test_photo_key_is_proxied_and_signed_url_never_reaches_browser",
        PHOTO_TESTS,
    ),
    (
        "M14 чек удаляется, даже если снимок остался в хранилище",
        RECEIPTS,
        '    if await _drop_photo_objects(p, [id], user):\n        raise HTTPException(status_code=503, detail="Storage is unavailable")',
        "    await _drop_photo_objects(p, [id], user)",
        "test_receipt_survives_when_storage_refuses_to_delete_photo",
        PHOTO_TESTS,
    ),
    (
        "M15 массовое удаление игнорирует незачищенные снимки",
        RECEIPTS,
        "    if blocked_storage:\n        deleted = [rid for rid in deleted if rid not in blocked_storage]",
        "    if blocked_storage:\n        pass",
        "test_bulk_delete_keeps_receipt_whose_photo_could_not_be_removed",
        PHOTO_TESTS,
    ),
    (
        "M16 снимок вообще не убирается из хранилища",
        RECEIPTS,
        "            await s3.delete_object(cfg, key)",
        "            pass",
        "test_deleting_receipt_removes_its_photo_from_storage",
        PHOTO_TESTS,
    ),
    (
        "M17 отказ хранилища при чтении маскируется под «фото нет»",
        RECEIPTS,
        'raise HTTPException(status_code=502, detail="Storage is unavailable")',
        'raise HTTPException(status_code=404, detail="No photo for this receipt")',
        "test_storage_failure_is_502_and_leaks_nothing",
        PHOTO_TESTS,
    ),
]
