...
ПРОПУСК = {
    ...
    # ─────────────────────────────────────────────────────────────────────
    # СТОРОЖ NDS-CLEANUP: запрос под проверкой information_schema.
    #
    # `_снять_колонки_ставок_ндс()` (app/database.py) СНАЧАЛА спрашивает
    # каталог, есть ли vat_20/vat_10, и `return`, если их нет. Только потом
    # идёт этот запрос. То есть на базе без колонок он НЕ ВЫПОЛНЯЕТСЯ никогда.
    #
    # sql_check про сторожа не знает и знать не может: он готовит КАЖДЫЙ
    # литеральный SQL из app/, не разбирая, под каким условием тот стоит.
    # Отсюда `UndefinedColumnError: столбец "vat_20" не существует` на чистой
    # базе — ложное срабатывание прибора, а не дефект кода.
    #
    # ⚠️ ПРОПУСК ИМЕННОЙ И УЗКИЙ, А НЕ «ИГНОРИРОВАТЬ ОШИБКИ КОЛОНОК».
    # Он совпадает с ОДНИМ конкретным запросом — по паре COALESCE(vat_20…)
    # и COALESCE(vat_10…) вместе. Любой другой запрос с несуществующей
    # колонкой прибор по-прежнему ловит; это проверено мутацией.
    #
    # ⚠️ ПЕРЕСМОТР — ПО СТРОКЕ ТРЕКЕРА, А НЕ ПО ИМЕНИ ЗДЕСЬ:
    #     docs/TASKS.md → T92 (репозиторий aocg-ai-office)
    # Закроется NDS-CLEANUP (колонки снимут окончательно и код уберут) —
    # пересматривается и этот пропуск. Образец — acceptedRebuildDiff
    # в design/app-icon.source.json фронта.
    "СТОРОЖ NDS-CLEANUP (T92)": lambda sql: (
        "COALESCE(vat_20,0)" in sql and "COALESCE(vat_10,0)" in sql
    ),
}
...
async def main():
    ...
    try:
        ...
        ошибки, проверено = [], 0
        пропущено = {имя: 0 for имя in ПРОПУСК}
    ...
    # ⚠️ ПРОПУСКИ ПЕЧАТАЮТСЯ ПОИМЕННО, А НЕ ОДНИМ ЧИСЛОМ. Одно число
    # не отличает «пропущен DDL» от «пропущен запрос, про который забыли»,
    # и читатель через месяц не увидит, ЧТО именно замолчали.
    print(f"Подготовлено запросов: {проверено}")
    for имя, сколько in пропущено.items():
        print(f"  пропущено «{имя}»: {сколько}")

    # ⚠️ ПРОПУСК, КОТОРЫЙ НИЧЕГО НЕ ЛОВИТ, — ЭТО ЛОЖЬ, А НЕ ТИШИНА.
    # Запрос переименовали или убрали, а правило осталось: прибор молчит
    # о том, чего уже нет, и продолжает выглядеть исправным. Тот же класс,
    # что T87 («совпало» без выполнения). DDL не проверяем — он есть всегда.
    холостые = [имя for имя, с in пропущено.items() if с == 0 and имя != "DDL"]
    if холостые:
        print(f"\n⚠️ ПРОПУСК НЕ СРАБОТАЛ НИ РАЗУ: {', '.join(холостые)}")
        print("   Правило есть, а запроса под ним нет. Либо запрос изменился,")
        print("   либо правило пора снять. Молчащий пропуск хуже отсутствующего.")
        sys.exit(1)
    if ошибки:
