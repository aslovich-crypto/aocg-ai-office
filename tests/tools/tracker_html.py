...
def разметка_строки(текст):
    ...
    ② жирное, ВНУТРИ которого лежит код («**… фронт `fe01d98` …**»),
      ...
      вместо 26.
    """
    # ⚠️ ЭКРАНИРОВАННЫЙ СТОЛБ СНИМАЕТСЯ ПЕРВЫМ И ВНУТРИ КОДА ТОЖЕ. В
    # markdown-таблице `\|` — единственное экранирование, которое работает
    # ДАЖЕ в кодовой вставке: столб режет строку раньше, чем разбирается
    # строчный код, поэтому иначе черту в ячейку не положить. Разбор
    # (md_table.ячейки) делит по `(?<!\\)\|` верно, но косую после него не
    # снимал НИКТО — и читатель видел «\|». Замер 06.09.2026: в ячейках
    # трекера 27 штук, на странице 2 вне кода и 23 внутри кодовых вставок.
    ч = html.escape(текст).replace("\\|", "|")
    коды = []
...
def без_разметки(текст):
    """Тот же текст без знаков разметки — для АТРИБУТОВ, где тегов быть не может.

    ⚠️ ЗАЧЕМ ОТДЕЛЬНО, А НЕ `разметка_строки`. Подсказка (`title`) живёт
    внутри тега, и HTML туда не положить. Раньше там стоял голый escape —
    и в подсказке усечённой ячейки «Факт» висело «⏱ учтено в [[T171]]»
    вместе со скобками. Показывать в подсказке ДРУГОЙ текст, чем в ячейке,
    нельзя: подсказка для того и есть, чтобы дочитать усечённое."""
    ч = текст.replace("\\|", "|")
    ч = re.sub(r"`([^`]+)`", r"\1", ч)
    ч = re.sub(r"\*\*(.+?)\*\*", r"\1", ч, flags=re.S)
    ч = re.sub(r"\[\[([^\]]+)\]\]", r"\1", ч)
    ч = re.sub(r"~~(.+?)~~", r"\1", ч, flags=re.S)
    return ч


_РЕФ = re.compile(r'<span class="ref">([^<]+)</span>')


def ссылки_на_карточки(кусок):
    """<span class="ref">ИД</span> → ссылка на якорь карточки #ИД.

    ⚠️ ОБЩИЙ РАЗБОР ДЕЛАЕТ ПОДПИСЬ, А В НАВИГАЦИИ НУЖНА ССЫЛКА. Сторож
    проверяет, что у каждого номера порядка есть href="#ид" и карточка
    с таким id. Поэтому подпись превращается в ссылку ПОСЛЕ разбора,
    а не вместо него: разбор остаётся один на всю витрину."""

    def _в_ссылку(м):
        ид = html.unescape(м.group(1))
        return (
            f'<a class="oref" href="#{html.escape(ид, quote=True)}">'
            f"{html.escape(ид)}</a>"
        )

    return _РЕФ.sub(_в_ссылку, кусок)
...
def пункты_блока(текст):
    """Пункты блока сырьём: [(номер, первая строка, продолжение), ...].

    ⚠️ ОДИН РАЗБОР НА ДВА ВЗГЛЯДА, А НЕ ДВА РАЗБОРА. Отсюда растут и
    `порядок_выполнения` (для витрины: номер + ссылки), и сверка блока
    с фактом (для сторожа: ей нужен ещё и признак зачёркивания). Завести
    второй разбор того же блока значило бы сделать ровно то, от чего
    лечит вся задача T39, — две копии одного смысла.

    ⚠️ ПЕРВАЯ СТРОКА И ПРОДОЛЖЕНИЕ РАЗДЕЛЕНЫ НАМЕРЕННО. Пункт часто несёт
    под собой абзац с доводом, а в доводе законно встречаются и `~~`,
    и ссылки на соседние строки. Всё, что решает о состоянии пункта,
    обязано читаться ТОЛЬКО из первой строки: два ложных срабатывания
    08.09.2026 (девятое наблюдение, случай ④) были именно тем, что сверка
    брала весь пункт целиком.
    """
    ...
    for строка in м.group(1).split("\n"):
        мн = re.match(r"^(\d+)\.\s+(.*)$", строка.strip())
        if мн:
            пункты.append([мн.group(1), мн.group(2), []])
        elif пункты and строка.startswith("   ") and строка.strip():
            пункты[-1][2].append(строка.strip())
    return [(н, п, "\n".join(х)) for н, п, х in пункты]
...
def порядок_выполнения(текст):
    ...
    return [
        (н, re.findall(r"\[\[([^\]]+)\]\]", первая), первая)
        for н, первая, _ in пункты_блока(текст)
    ]
...
def блок_порядка(пункты):
    ...
    ⚠️ РАЗМЕТКУ РАЗБИРАЕТ ОБЩИЙ `разметка_строки`, А НЕ СВОЙ escape. До
    06.09.2026 здесь стоял голый html.escape с ручной подменой [[ид]] —
    и пункты печатались сырьём. Замер того дня: 22 сырых знака из 26
    (** 18, ~~ 4) шли ИМЕННО ОТСЮДА; ноль до `db6d9b0`, дальше рост
    девятью коммитами за один день — блок порядка тогда и завели.
    Разбор всё это время лежал в двадцати строках выше и умел ровно то,
    чего не хватало; его просто не звали.

    ⚠️ ПОЧЕМУ ЧИНИЛСЯ ПРИБОР, А НЕ ТЕКСТ (решение владельца 06.09.2026):
    жирное отделяет номер задачи от пояснения, без него пункты сливаются,
    а [[ид]] в ячейке — законная ссылка. Прибор, ради которого надо писать
    хуже, неисправен — тот же класс, что сторожа, которые в тот день
    трижды мерили не то."""
    if not пункты:
        return ""
    строки = []
...
def строка_задачи(запись):
    ...
    # ⚠️ СРОКИ ТОЖЕ РАЗБИРАЮТСЯ. В «Факте» двух строк лежит ссылка —
    # «⏱ учтено в [[T171]]» и «⏱ учтено в [[T39]]», — и она печаталась
    # сырьём в ОБОИХ местах показа: здесь и в усечённой ячейке строки.
    # Это оставшиеся 4 сырых знака из 26 (замер 06.09.2026).
    шапка_сроков = f'<p class="сроки">{разметка_строки(сроки)}</p>' if сроки else ""
    ...
    return (
        ...
        + (
            f'<span class="fact" '
            f'title="{html.escape(без_разметки(сроки), quote=True)}">'
            f"{разметка_строки(сроки)}</span>"
            if сроки
            else ""
        )
...
def сводка(задачи):
    """Два ПОДПИСАННЫХ ряда чисел. Считаются, а не проставляются.

    ⚠️ ДВА РЯДА, А НЕ ОДИН, И ЭТО НЕ ОФОРМЛЕНИЕ. До 06.09.2026 шесть плиток
    стояли ряд в ряд и выглядели одним разрезом, а считали РАЗНОЕ: статусы —
    по своим срезам, приоритеты — ПО ВСЕЙ БАЗЕ, включая закрытые. Владелец
    прочёл их как один разрез и спросил, почему 93+171+73 даёт все задачи, а
    «к работе» — только часть. Плитка не должна требовать держать в голове,
    что она считает.

    ⚠️ И «ОТКРЫТО» ВМЕСТО «К РАБОТЕ». Прежнее число считало ТОЛЬКО ⬜ и
    молчало про 🔵, ⏸ и 👀 — на 06.09.2026 это 32 задачи, треть очереди,
    прочитанная как «сделано». Теперь под числом стоит разбивка.
    """
    ...
    открыто = sum(ст.get(з, 0) for з in ("⬜", "🔵", "⏸", "👀"))
    разбивка = " · ".join(
        f"{з} {ст.get(з, 0)}" for з in ("⬜", "🔵", "⏸", "👀") if ст.get(з, 0)
    )
    состояние = (
        ("", открыто, "открыто", разбивка),
        ("", ст.get("✅", 0), "✅ закрыто", ""),
        ("", с_фактом, "из них с фактом", "подмножество закрытых"),
    )
    приоритет = (
    ...
    def ряд(карточки):
        return "".join(
            f'<div class="stat {к}"><b>{ч}</b><span>{п}</span>'
            + (f'<i class="hint">{х}</i>' if х else "")
            + "</div>"
            for к, ч, п, х in карточки
        )

    return (
        '<div class="statgroup"><h3>Состояние · по срезам статуса</h3>'
        f'<div class="stats">{ряд(состояние)}</div></div>'
        '<div class="statgroup"><h3>Приоритет · ПО ВСЕЙ БАЗЕ, включая закрытые</h3>'
        f'<div class="stats">{ряд(приоритет)}</div>'
        '<p class="statnote">Сумма трёх приоритетов равна числу ВСЕХ задач — '
        "это другой разрез, чем «открыто» и «закрыто» выше.<br>"
        "⚠️ 06.09.2026 числа выросли не потому, что задач стало больше, а "
        "потому что разбор стал их ВИДЕТЬ: <b>337 → 373</b>. Прежняя регулярка "
        "требовала, чтобы ID кончался цифрой, и теряла 35 задач молча (T188)."
        "</p></div>",
        пр,
    )
...
.stat .hint{display:block; margin-top:4px; font-size:10px; font-style:normal;
  color:var(--muted); letter-spacing:.02em; text-transform:none}
.statgroup{margin:22px 0 0}
.statgroup h3{margin:0 0 8px; font-size:11px; font-weight:600; letter-spacing:.07em;
  text-transform:uppercase; color:var(--muted)}
.statgroup .stats{margin:0}
.statnote{margin:10px 0 0; font-size:12px; line-height:1.5; color:var(--muted)}
.bar{
