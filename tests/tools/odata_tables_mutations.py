...
МУТАНТЫ = [
    (
        "⑰ умолчание политики проведения снова «when_mapped»",
        РЕЛЬСЫ,
        ...
        "Клиент, не выбравший политику, получил бы проведение молча — то есть "
        "мы приняли бы за него бухгалтерское решение в его учёте",
    ),
    (
        "⑱ первичный ключ остался со старым именем таблицы",
        РЕЛЬСЫ,
        '''    """ALTER INDEX IF EXISTS odata_category_map_pkey
    RENAME TO org_category_map_pkey""",\n''',
        "",
        [ЖИВОЙ],
        "индекс `odata_category_map_pkey` у таблицы `org_category_map` — "
        "ложный след: следующий разбор пойдёт искать таблицу, которой нет",
    ),
    (
