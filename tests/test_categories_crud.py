...
def _default_cat_id(db, org_id=1):
    return next(
        ...
        if c.get("org_id") == org_id and c.get("is_default")
    )
...
async def test_get_categories_nested_structure(client, db):
    ...
    assert {"id", "name", "tax_kind", "position", "is_default", "is_visible"} <= set(
        sample
    )
...
async def test_category_create_as_admin_ok(client, db):
    ...
    resp = await client.post(
        "/api/categories/", json={"name": "Моя статья", "group_id": _group_id(db)}
    )
    assert resp.status_code == 200
    body = resp.json()
...
async def test_category_create_as_accountant_ok(client_accountant, db):
    ...
    resp = await client_accountant.post(
        "/api/categories/",
        json={"name": "Статья бухгалтера", "group_id": _group_id(db)},
    )
    assert resp.status_code == 200
...
async def test_category_create_duplicate_409(client, db):
    await seed_default_categories(db, 1)
    resp = await client.post(
        "/api/categories/", json={"name": "Топливо", "group_id": _group_id(db)}
    )
...
async def test_category_create_foreign_group_404(client, db):
    ...
    resp = await client.post(
        "/api/categories/", json={"name": "Чужая группа", "group_id": foreign}
    )
...
async def test_category_create_invalid_tax_kind_400(client, db):
    ...
    resp = await client.post(
        "/api/categories/",
        json={"name": "Плохой вид", "group_id": _group_id(db), "tax_kind": "Чепуха"},
    )
...
async def test_category_create_explicit_tax_kind(client, db):
    ...
    resp = await client.post(
        ...
        json={
            "name": "Материальная",
            "group_id": _group_id(db),
            "tax_kind": "Материальные расходы",
        },
    )
    assert resp.status_code == 200
...
async def test_category_patch_rename_and_tax_kind_ok(client, db):
    ...
    cid = (
        await client.post(
            "/api/categories/", json={"name": "Старое", "group_id": _group_id(db)}
        )
    ).json()["id"]
    resp = await client.patch(
        f"/api/categories/{cid}",
        json={"name": "Новое", "tax_kind": "Материальные расходы"},
    )
    assert resp.status_code == 200
    body = resp.json()
...
async def test_category_patch_system_403(client, db):
    await seed_default_categories(db, 1)
    resp = await client.patch(
        f"/api/categories/{_default_cat_id(db)}",
        json={"name": "Переименовать системную"},
    )
    assert resp.status_code == 403
...
async def test_category_patch_invalid_tax_kind_400(client, db):
    ...
    cid = (
        await client.post(
            "/api/categories/", json={"name": "Для правки", "group_id": _group_id(db)}
        )
    ).json()["id"]
    resp = await client.patch(
        f"/api/categories/{cid}", json={"tax_kind": "Несуществующий"}
    )
    assert resp.status_code == 400
...
async def test_category_delete_ok(client, db):
    ...
    cid = (
        await client.post(
            "/api/categories/", json={"name": "На удаление", "group_id": _group_id(db)}
        )
    ).json()["id"]
...
async def test_category_delete_with_receipts_409(client, db):
    ...
    cid = (
        await client.post(
            "/api/categories/", json={"name": "С чеками", "group_id": _group_id(db)}
        )
    ).json()["id"]
...
async def test_category_visibility_hide_show_system(client, db):
    ...
    hide = await client.patch(
        f"/api/categories/{cid}/visibility", json={"is_visible": False}
    )
    assert hide.status_code == 200
    assert hide.json()["is_visible"] is False
    show = await client.patch(
        f"/api/categories/{cid}/visibility", json={"is_visible": True}
    )
