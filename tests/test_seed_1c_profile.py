...
def test_непринимаются_достаётся_ровно_пяти_категориям():
    """⚠️ ПЯТЬ, И КАЖДАЯ ПО СВОЕЙ НОРМЕ (решение владельца 21.09.2026):
    питание сотрудников — п. 25 ст. 270 НК (не закреплено договором),
    подарки — п. 16 ст. 270 НК, «не учитываемые» — смысл самого вида.
    Шестая такая категория означала бы, что налог занижен или завышен
    молча."""
    м = _засев()
    словарь = _пары_словаря()
    по_категории = {имя: признак for имя, _с, _р, признак in м.ПЕРЕОПРЕДЕЛЕНИЯ}
    по_виду = {вид: признак for вид, _с, _р, признак, _почему in м.ПРАВИЛА_ВИДОВ}
    признаки = {
        имя: по_категории.get(имя, по_виду[вид]) for имя, вид in словарь.items()
    }
    непринимаются = sorted(и for и, п in признаки.items() if п == "НеПринимаются")
