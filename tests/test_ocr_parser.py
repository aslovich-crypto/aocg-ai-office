...
def test_parse_ocr_response_datetime_to_object():
    ...
    assert (
        p["datetime"].year == 2026
        and p["datetime"].month == 5
        and p["datetime"].day == 26
    )
...
def test_parse_ocr_items_vat_rate_string():
    items = parse_ocr_items(
        {"items": [{"name": "X", "vat_rate": "20"}, {"name": "Y", "vat_rate": None}]}
    )
...
# ─── E. ОСНОВАНИЕ УЗКОЙ ФОРМЫ ВАРИАНТА A (строка 31, ③, 12.09.2026) ────
#
# ⚠️ ЗАЧЕМ ЭТОТ ТЕСТ СУЩЕСТВУЕТ. С 12.09.2026 бэкенд СОХРАНЯЕТ ФН, пришедший
# в теле запроса, даже когда ярлык источника — photo_ocr: ярлык описывает
# последний шаг, а не происхождение поля, и по пути «QR → ФНС молчит → фото»
# там лежит машинно прочитанный номер, а не угаданный.
#
# ⚠️ ВСЯ БЕЗОПАСНОСТЬ ЭТОГО РЕШЕНИЯ ДЕРЖИТСЯ НА ОДНОМ УТВЕРЖДЕНИИ: распознавание
# ФН не отдаёт вовсе. Пока это так, подставить в реквизит угаданную цифру
# неоткуда. Перестанет быть так — и защита исчезнет МОЛЧА, потому что снаружи
# ничего не изменится. Поэтому утверждение проверяется, а не подразумевается.


def test_ocr_не_умеет_отдавать_фн():
    """Промпт запрещает читать ФН, и запрет обязан остаться на месте."""
    import pathlib

    исходник = (
        pathlib.Path(__file__).resolve().parents[1] / "app" / "routers" / "ocr.py"
    ).read_text(encoding="utf-8")
    assert "НЕ извлекай ФН" in исходник, (
        "запрет на чтение ФН исчез из промпта — вместе с ним исчезло основание "
        "узкой формы Варианта A (строка 31)"
    )
