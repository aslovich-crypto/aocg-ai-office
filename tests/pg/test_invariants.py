...
@pytest.mark.asyncio
async def test_пустая_почта_невозможна_вовсе(db, seeded):
    """⚠️ ЭТОТ ТЕСТ ЗАМЕНИЛ СВОЮ ПРОТИВОПОЛОЖНОСТЬ 08.09.2026, и это надо
    знать, читая историю. До того здесь стояло
    `test_пустая_почта_не_блокирует_вторую_пустую`: индекс был ЧАСТИЧНЫМ
    именно потому, что безпочтовых людей могло быть много.

    Теперь у `users` стоит CHECK на непустоту почты — второй пустой не будет,
    потому что не будет и первой. Частичность индекса снята тем же заходом:
    её условие стало истинным всегда.
    """
