...
async def test_photo_ocr_parses_raw_data_into_columns_and_items(client, db):
    ...
    # ⚠️ ПРОВЕРКА ПЕРЕПИСАНА 11.09.2026 — ОНА СТАЛА ТАВТОЛОГИЕЙ ОТ НАШЕЙ ЖЕ
    # ПРАВКИ, И ЭТО СТОИТ ЗНАТЬ В ЛИЦО. До миграции колонка была TIMESTAMPTZ,
    # и здесь стоял перевод `$1::timestamptz AT TIME ZONE current_setting(...)`:
    # он был нужен, чтобы проверка была верна и на прод-кластере (UTC), и на
    # локальном (Europe/Moscow), где то же 12:41 читалось как 09:41+00:00.
    # После смены типа на TIMESTAMP WITHOUT TIME ZONE значение НАИВНО, и тот
    # перевод стал круговым: замер 11.09 в обоих поясах даёт 12:41 при любом
    # содержимом колонки. **Зелёная проверка, которая не могла покраснеть.**
    #
    # ⚠️ ЧЕСТНАЯ ФОРМА ПОСЛЕ МИГРАЦИИ ПРОЩЕ, И В ЭТОМ ВЕСЬ СМЫСЛ РАБОТЫ:
    # колонка больше не обещает зоны, значит в ней лежит ровно то, что
    # напечатала касса, и сравнивать надо прямо с этим. Заодно проверка стала
    # прибором на саму миграцию: верни тип обратно на локальном кластере —
    # значение станет 09:41 и строка покраснеет.
    из_базы = (await db.чек(body["id"]))["datetime"]
    assert body["datetime"] == из_базы.isoformat()
    assert из_базы.tzinfo is None, (
        "колонка обязана быть БЕЗ зоны: TIMESTAMPTZ обещает зону, которой "
        "касса не присылала (строка 20)"
    )
    assert из_базы.isoformat().startswith("2026-05-26T12:41"), (
        "в колонке лежит стенное время кассы, а не переведённое: "
        f"пришло {из_базы.isoformat()}"
    )
...
# ─── строка 31 · канал реквизитов ФНС ─────────────────────────────────
#
# ⚠️ ПОЧЕМУ ЭТИ ТЕСТЫ ЖИВУТ НА ЖИВОЙ БАЗЕ, А НЕ НА ДВОЙНИКЕ. Предмет проверки —
# то, что ДОЕХАЛО ДО КОЛОНКИ, а не код ответа. Ручка отвечает 200 в обоих
# случаях: и когда значение записано, и когда оно молча отброшено, — ровно
# этим и объяснялись 76 чеков без реквизитов.


@pytest.mark.asyncio
async def test_время_чека_от_клиента_доезжает_до_колонки(client, db):
    """Правка ⑤ строки 31. Без времени право на дозапрос не возвращается.

    Запрос к ФНС — это строка QR, и в ней `t=` дата и время ДО МИНУТЫ.
    Время писалось только из разобранного ответа ФНС, то есть только когда
    ФНС уже ответила, — а нужно оно ровно в обратном случае.
    """
    payload = {
        "date": "2026-06-11",
        "org": "МЕРКА",
    ...
    строка = await db.pool.fetchrow(
        "SELECT datetime, kkt_fn, fd_num, fpd FROM receipts WHERE id=$1",
        resp.json()["id"],
    )
    assert строка["datetime"] is not None, "время чека не доехало до колонки"
    # ⚠️ ЗОНЫ НЕТ И НЕ ДОЛЖНО БЫТЬ: стенное время кассы, пояс ККТ неизвестен.
    assert строка["datetime"].tzinfo is None
    assert строка["datetime"].hour == 11 and строка["datetime"].minute == 45
    # И три реквизита рядом — иначе проверка времени бессмысленна.
    assert строка["kkt_fn"] == "7380440902249741"
    assert строка["fd_num"] == "43194"
    assert строка["fpd"] == "788765239"


@pytest.mark.asyncio
async def test_чек_по_QR_при_молчащей_ФНС_получает_право_на_дозапрос(client):
    """Замыкающая проверка правки ⑤: пять значений собраны — право вернулось.

    До 12.09.2026 такой чек залипал навсегда: три реквизита есть, времени нет,
    `_можно_дозапросить` отвечает False, кнопка не показывается, фоновый
    дозапрос чек не берёт. Признак приходит в ответе ручки, поэтому проверяем
    его там же, где его читает фронт.
    """
    payload = {
        "date": "2026-06-11",
        "org": "МЕРКА",
    ...
    assert detail.json()["можно_дозапросить"] is True


@pytest.mark.asyncio
async def test_без_времени_права_на_дозапрос_нет(client):
    """Обратная половина пары: убрали одно значение — признак обязан погаснуть.

    Без неё зелёный тест выше не отличить от «признак всегда True».
    """
    payload = {
        "date": "2026-06-11",
        "org": "МЕРКА",
