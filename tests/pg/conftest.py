...
class ЖиваяБаза:
    ...
    async def добавить_пользователя(
        ...
        # ⚠️ УМОЛЧАНИЯ ДЛЯ ОБЯЗАТЕЛЬНЫХ ПОЛЕЙ (08.09.2026). У `users` встали
        # NOT NULL и CHECK на непустоту имени, фамилии и почты, а тесты заводят
        # людей десятками и про имена в большинстве случаев не спрашивают.
        # Подставляем РАЗЛИЧИМЫЕ значения с id внутри: одинаковые почты сцепили
        # бы людей уникальным индексом, а одинаковые имена прятали бы промах
        # прибора по соседнему человеку — тот самый «чужой узел», только
        # в данных. Тест, которому важно ИМЕННО пустое поле, передаёт его явно
        # и получит отказ базы — это его предмет, а не помеха.
        if first_name is None:
            first_name = "Тест"
        if last_name is None:
            last_name = f"Тестов{id}"
        if email is None:
    ...
    async def обеспечить_пользователя(self, id, **поля):
        """Пользователь ЕСТЬ — завести, если его ещё нет.

        ⚠️ Отдельным именем, а не `ON CONFLICT` внутри `добавить_пользователя`:
        «завести» и «убедиться, что есть» — разные намерения. Первое обязано
        падать на дубле (иначе тест, дважды заводящий одного человека, молчит
        о своей ошибке), второе для фикстур, которые лишь гарантируют наличие
        смотрящего и не знают, не завёл ли его уже `seeded`."""
        есть = await self.pool.fetchval("SELECT 1 FROM users WHERE id=$1", id)
        if not есть:
    ...
    async def добавить_чек(
        ...
        # ⚠️ `source`, `org_inn` и `created_at` — не украшение: дедуп чеков
        # решает ИМЕННО по ним. Ветка 0 («двойной тап») смотрит на `created_at`,
        # ветки 2/3 различаются наличием `org_inn`. У двойника поля просто
        # лежали в словаре; здесь они колонки.
        #
        # ⚠️ КОЛОНКИ `category` ЗДЕСЬ НЕТ НАМЕРЕННО. Замер прода 06.09.2026
        # (бастион): `column "category" does not exist` — на бою есть только
        # `category_id` и `category_manual`, а `init_db` колонку всё ещё
        # создаёт, и тестовые базы её получают. Заход 1 писал в неё, потому что
        # так делал двойник; замер показал, что НИ ОДИН из 61 теста на неё не
        # опирается — приложение строку в колонку не пишет с «варианта B»
        # (см. app/routers/receipts.py:420). Расхождение схемы init_db и прода
        # чинится отдельной строкой владельца, здесь — просто не пишем того,
        # чего на бою нет.
        await self.pool.execute(
            "INSERT INTO receipts (id, org, amount, date, payment, kkt_fn, fd_num, "
            "org_id, user_id, org_brand, org_legal, source, org_inn, "
    ...
    async def момент(self, **сдвиг):
        ...
        в процессе даёт UTC. Совпадут они или нет — зависит от МАШИНЫ: локальный
        кластер поднимается в поясе системы (здесь Europe/Moscow, замер показал
        расхождение ровно в три часа — чек, засеянный «прямо сейчас», ложился
        трёхчасовой давности), а прод стоит в UTC, и там `now()` и `utcnow()`
        сходятся (замер прода 06.09 через бастион, владелец).

        ⚠️ ИМЕННО ПОЭТОМУ ЗАСЕВ ИДЁТ ПО ЧАСАМ БАЗЫ, А НЕ ПО ЧАСАМ ПРОЦЕССА.
        Окна дедупа считаются от `NOW()` (90 секунд и 7 дней). Тест, опирающийся
        на `utcnow()`, верен только там, где кластер в UTC, и тихо смещает
        проверяемое окно везде, где нет. Довод не в том, что на проде дефект —
        его там нет; довод в том, что тест обязан быть верен В ОБЕИХ средах.
        """
...
async def seeded(db):
    ...
    # ⚠️ «Обеспечить», а не «завести»: сюиту может сопровождать автоиспользуемая
    # фикстура смотрящих, и Иван к этому моменту уже заведён ею. Порядок фикстур
    # pytest не обещает, а падение на дубле здесь означало бы не дефект, а
    # совпадение двух засевов одного и того же человека.
