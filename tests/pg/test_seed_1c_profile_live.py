...
@pytest.mark.asyncio
async def test_засев_кладёт_профиль_девять_правил_и_переопределения(
    db, адрес_живой_базы
):
    м = _засев()
...
@pytest.mark.asyncio
async def test_после_засева_ни_одна_категория_не_осталась_без_статьи(
    db, адрес_живой_базы
):
    """⚠️ ГЛАВНАЯ СВЯЗКА ЗАСЕВА С РЕАЛЬНОСТЬЮ (1C-28). До 21.09.2026 три вида
    расхода оставались без статьи — их не было в базе клиента, и чеки таких
    категорий уезжали «без соответствия». Теперь не должно остаться ни одной:
    категория без статьи означает, что 1С подставит свою, молча."""
    м = _засев()
    await _организация_с_категориями(db)
    assert await м.прогнать(адрес_живой_базы, закрепить=True) == 0

    раскладка = await db.pool.fetch(м.РАСКЛАДКА_SQL % 1)
    assert len(раскладка) == 48, len(раскладка)
    без_статьи = [с["категория"] for с in раскладка if not с["статья"]]
    assert not без_статьи, "без статьи остались: %s" % без_статьи

    по_правилу = {}
    for с in раскладка:
        по_правилу.setdefault(с["правило"], []).append(с["категория"])
    assert len(по_правилу["по категории"]) == len(м.ПЕРЕОПРЕДЕЛЕНИЯ) == 34
    assert len(по_правилу["по виду"]) == 14, sorted(по_правилу["по виду"])

    # ⚠️ «НеПринимаются» — РОВНО ПЯТЬ КАТЕГОРИЙ, каждая по своей норме
    # (решение владельца 21.09.2026): питание сотрудников — п. 25 ст. 270 НК,
    # у бюро не закреплено договором; подарки — п. 16 ст. 270 НК; пятая —
    # сам вид «не учитываемые». Шестая означала бы налог, занижённый молча.
    непринимаются = sorted(
        с["категория"] for с in раскладка if с["признак"] == "НеПринимаются"
    )
    assert непринимаются == [
    ...
    # Статья у питания своя, а не «Прочие затраты»: признак про налог,
    # статья про то, где расход виден в учёте.
    питание = [с for с in раскладка if с["категория"] == "Обеды сотрудников"][0]
    assert питание["статья"] == "Питание и продукты для офиса"


@pytest.mark.asyncio
async def test_переопределение_главнее_правила_вида(db, адрес_живой_базы):
    """§ 5.4 на живых данных: «Такси и каршеринг» по виду расхода ушло бы
    в «Прочие затраты», переопределение ведёт его в «Транспорт сотрудников»."""
    м = _засев()
    await _организация_с_категориями(db)
    assert await м.прогнать(адрес_живой_базы, закрепить=True) == 0
    раскладка = {с["категория"]: с for с in await db.pool.fetch(м.РАСКЛАДКА_SQL % 1)}
    такси = раскладка["Такси и каршеринг"]
    assert такси["статья"] == "Транспорт сотрудников"
    assert такси["правило"] == "по категории"
    # Пара: категория без переопределения идёт по виду расхода.
    доставка = раскладка["Доставка товаров"]
    assert доставка["статья"] == "Транспортные расходы"
    assert доставка["правило"] == "по виду"
