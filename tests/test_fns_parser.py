...
def test_коды_пять_и_шесть_не_склеиваются_в_позициях():
    """ВТОРОЕ место склейки, и оно переживёт починку первого.

    ⚠️ `_VAT_RATE_BY_CODE` отображает и код 5 («НДС 0%»), и код 6 («Без НДС»)
    в одну строку "0" — то есть в колонке `receipt_items.vat_rate` они
    неразличимы. Фронт при этом их РАЗЛИЧАЕТ (`TAG_1199`, комментарий
    «НДС 0% ≠ Без НДС»), а `VAT_RATE` кода 6 не знает вовсе и отбрасывает
    такую позицию из разбивки. Три трактовки одной пары кодов на один чек.

    ⚠️ ТЕСТ ФИКСИРУЕТ ТЕКУЩЕЕ ПОЛОЖЕНИЕ, А НЕ НОРМУ: сегодня склейка ЕСТЬ,
    и здесь это записано как факт. Разведут коды — тест покраснеет, и это
    будет ПРАВИЛЬНЫЙ красный: тогда его переписывают вместе с починкой,
    а не «возвращают как было».
    """
    from app.parsers.fns_parser import _VAT_RATE_BY_CODE

    assert _VAT_RATE_BY_CODE[5] == _VAT_RATE_BY_CODE[6] == "0", (
        "СНИМОК: сегодня коды 5 и 6 склеены в позициях. Красный здесь означает, "
        "что их развели — перепишите тест вместе с починкой, а не наоборот"
    )
