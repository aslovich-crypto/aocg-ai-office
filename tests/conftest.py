...
# ⚠️ ПОТОЛОК ОСЛАБЛЕНИЯ СНИМАЕТСЯ ЗДЕСЬ ЯВНО, И ТОЛЬКО ЗДЕСЬ (S-49).
# С 04.09.2026 значение строгого лимита из ОКРУЖЕНИЯ режется потолком
# (20/мин): панель может ужесточать защиту, но не ослаблять. Тесты бьют
# сотни запросов к /api/auth/* с одного адреса и на потолке падают
# с 429 — что и случилось при первом прогоне, 149 красных.
#
# ⚠️ ПОЧЕМУ ПОДМЕНА КОНСТАНТЫ, А НЕ «ВЫКЛЮЧАТЕЛЬ» В КОДЕ. Любая
# переменная вида AOCG_TEST_MODE, снимающая потолки, — это и есть класс
# S-49: её можно задать в панели, и защита исчезнет молча. Подмена живёт
# в тестовом коде, в прод-образ не попадает и видна тому, кто читает тесты.
import aocg_security.middleware as _мидлварь  # noqa: E402

_мидлварь.ПОТОЛОК_СТРОГОГО_ЛИМИТА = 100000
_мидлварь.ПОТОЛОК_ПОРОГА_БАНА = 100000
...
class FakePool:
    ...
    async def fetch(self, query, *args):
        ...
        # без ветки вовсе"». Черновик «бывший T184» («двойник под 427 тестами»)
    ...
    async def fetchrow(self, query, *args):
        ...
        if q.startswith("INSERT INTO receipts"):
            ...
            row = dict(
                ...
                photo_key=args[31],
                # №30: две величины вместо двусмысленной vat_0. Добавлены
                # ПОСЛЕДНИМИ в INSERT, поэтому позиции 0..31 не сдвинулись
