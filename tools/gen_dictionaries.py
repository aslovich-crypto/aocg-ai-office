...
def порождённые_наборы() -> list:
    ...
    Наборы здесь РАЗНОРОДНЫЕ (режимы, коды ФНС, роли) намеренно: общее у них
    не предмет, а дорога — порождение из кода бэкенда со штампом.
    """
    return [
        (ТАКСЫ_ОРГ_JS_ОТН, режимы_организации(), "TAX_SYSTEMS", ШАПКА_ОРГ),
        (ТАКСЫ_ЧЕК_JS_ОТН, коды_сно_чека(), "FNS_TAX_CODES", ШАПКА_ЧЕК),
        (РОЛИ_JS_ОТН, роли(), "ROLES", ШАПКА_РОЛИ),
    ]
...
# ─── РОЛИ: ОДИН ИСТОЧНИК, СЛОВАРЬ СВЕРЯЕТ (T39, заход 3) ──────────────────
#
# ⚠️ ВТОРОГО ИСТОЧНИКА НЕ ЗАВОДИМ — решение владельца 06.09.2026. Белый
# список ролей уже единственный и назван таковым: `ROLES` в app/auth.py
# (S-24). По нему валидируется всё, что заводит человека; мимо него в
# `users.role` приезжает произвольная строка, а гейты сравнивают роль
# РАВЕНСТВОМ — то есть «Admin» с большой буквы молча получает права рядового
# сотрудника, и разбираться будут на живом человеке.
#
# ⚠️ ПОРОЖДЁННЫЙ ФАЙЛ — ЭТО НЕ КОПИЯ ДЛЯ ЭКРАНОВ, А ЭТАЛОН ДЛЯ СТОРОЖА.
# Ни один экран его не импортирует, и так задумано: подписи ролей и их
# описания остаются во фронте (App.jsx), сторож лишь сверяет, что набор
# идентификаторов там ТОТ ЖЕ. Ровно это и значит «словарь СВЕРЯЕТ, а не
# копирует»: копия живёт для проверки, а не вместо экрана.
#
# ⚠️ `manager` СЮДА НЕ ВХОДИТ НАМЕРЕННО. Роль РП из «Финансов» в Приме никто
# не понимает, прав ей не даёт ни один гейт (app/auth.py:102). Добавлять —
# вместе с FIN-01, а не потому, что список показался неполным.
РОЛИ_JS_ОТН = os.path.join("lib", "roles.js")


def роли() -> list:
    """Канон ролей ИЗ КОДА: `ROLES` в app/auth.py, он же S-24.

    ⚠️ ЗАОДНО СВЕРЯЕМ ДВА ОБЪЯВЛЕНИЯ В ОДНОМ ФАЙЛЕ. Роли записаны там дважды:
    кортежем `ROLES` (по нему валидируются формы) и `Literal` в типе `Role`
    (по нему pydantic отвергает запрос). Разойдись они — половина входов
    приняла бы роль, которую вторая половина не признаёт, и молча. Порядок
    не сверяем: у кортежа он есть, у Literal он же, но смысла в нём нет.

    Замер 08.09.2026: дописал роль в `ROLES`, не тронув `Role`, — генерация
    останавливается и называет расхождение поимённо. До этой сверки такое
    не ловило ничто: файл один, а объявления в нём два.
    """
    _подключить_бэкенд()
    from typing import get_args
    ...
    из_литерала = set(get_args(Role))
    if из_литерала != set(ROLES):
        raise SystemExit(
            "ИСТОЧНИК САМ С СОБОЙ НЕ СХОДИТСЯ: app/auth.py, "
            f"ROLES={sorted(ROLES)}, Literal Role={sorted(из_литерала)}"
        )
    return list(ROLES)
...
def main() -> int:
    ...
    if not проверка:
        ...
        if фронт_доступен:
            ...
            for отн, набор, имя, шапка in порождённые_наборы():
            ...
            _причесать_фронт(
                ...
                + [os.path.join(ФРОНТ_SRC, о) for о, *_ in порождённые_наборы()]
            )
        else:
    ...
    for отн, набор, имя, _ in порождённые_наборы():
        путь = os.path.join(ФРОНТ_SRC, отн)
        try:
