...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],
    # ⚠️ БЕЗ ЭТОЙ СТРОКИ ИМЯ ВЫГРУЖЕННОГО ФАЙЛА ПРИШЛОСЬ БЫ ЗАДАВАТЬ ДВАЖДЫ.
    # Браузер по умолчанию не отдаёт скрипту НИ ОДНОГО заголовка ответа кроме
    # простых, поэтому `Content-Disposition` для клиента не существует, и фронт
    # вынужден был бы собирать имя файла сам — второй источник той же истины,
    # расходящийся молча. Список именно перечислением, а не `*`: звёздочка
    # здесь открывает ВСЕ заголовки ответа, а нам нужен ровно один.
