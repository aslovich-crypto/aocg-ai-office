...
def тело(значение) -> str:
    """Значение, написанное ЧЕЛОВЕКОМ, — в тело письма безопасно.

    ⚠️ ЗАЧЕМ, РЕШЕНИЕ ВЛАДЕЛЬЦА 10.09.2026: «гигиена, а не безопасность
    высокого полёта». Название отчёта и причину отклонения печатает человек,
    и письмо собирается СТРОКОЙ — угловая скобка в «Отчёт <май>» рвёт
    вёрстку письма, а в худшем случае подставляет разметку в чужой ящик.
    Экранируется всё, что пришло от человека: название, причина, имя,
    продавец, название организации.

    ⚠️ ССЫЛКИ НЕ ЭКРАНИРУЮТСЯ ЭТОЙ ФУНКЦИЕЙ НАМЕРЕННО: они собираются нами
    из `APP_URL` и токена, человек их не пишет. Экранируй мы их здесь —
    `&` в будущем параметре превратился бы в `&amp;` внутри href, и ссылка
    перестала бы работать. Разные вещи разными руками.
    """
    return html.escape(str(значение or ""), quote=True)


def тема(значение) -> str:
    """То же для ТЕМЫ письма — но задача другая, и подмена другая.

    ⚠️ В ТЕМЕ ЭКРАНИРОВАТЬ HTML НЕЛЬЗЯ: тема не разметка, и `&amp;` человек
    прочтёт буквально. Опасность здесь иная — ПЕРЕВОД СТРОКИ: заголовок
    письма им заканчивается, и `Subject: Отчёт\nBcc: чужой@адрес` это
    подделка заголовков. `EmailMessage` такое отвергает исключением, причём
    сборка письма стоит ВНЕ `try` (`_send` ниже), то есть падение улетело бы
    наружу и уронило бы запрос, который письмо всего лишь сопровождает.
    Поэтому переводы строк и управляющие символы схлопываем в пробел.
    """
...
def send_report_status_email(
    ...
    html = f"""<div style="font-family:Arial,sans-serif;color:#111318">
      ...
      <p>Отчёт <b>{тело(отчёт)}</b> — {статус.lower()}.</p>
      {объяснение}
      {что_делать}
      {кнопка}
    </div>"""
    return _send(to_email, тема(f"{заголовок}: {отчёт} — AOCG AI Офис"), html)
...
def send_report_submitted_email(
    ...
    """
    строка_суммы = f"<p>Сумма: <b>{тело(сумма)}</b></p>" if сумма else ""
    кнопка = (
        f'<p><a href="{ссылка}" style="background:#A4161A;color:#fff;padding:12px 22px;'
    ...
    html = f"""<div style="font-family:Arial,sans-serif;color:#111318">
      ...
      <p><b>{тело(автор)}</b> отправил отчёт <b>{тело(отчёт)}</b>.</p>
      {строка_суммы}
      {кнопка}
    </div>"""
    return _send(to_email, тема(f"Отчёт на проверку: {отчёт} — AOCG AI Офис"), html)


def send_invite_accepted_email(to_email: str, кто: str, ссылка: str = "") -> bool:
    """Управляющему: по приглашению завелась учётная запись (T159, событие №4).

    ⚠️ ТЕКСТ ГОВОРИТ ПРО УЧЁТНУЮ ЗАПИСЬ, А НЕ ПРО РАБОТУ, И ЭТО ПРАВЛЕНО
    10.09.2026. Было «принял приглашение и завёл учётную запись» — обещание,
    верное лишь наполовину: при включённой почте письмо уходит в момент
    создания записи, когда человек ЕЩЁ НЕ ВОШЁЛ и может не подтвердить адрес
    никогда. Тема письма и заголовок события говорят одно и то же — иначе
    два канала одного события разошлись бы в словах.
    """
    кнопка = f'<p><a href="{ссылка}">Список сотрудников</a></p>' if ссылка else ""
    html = f"""<div style="font-family:Arial,sans-serif;color:#111318">
      <h2 style="color:#A4161A">Новый сотрудник</h2>
      <p><b>{тело(кто)}</b> завёл учётную запись по вашему приглашению.</p>
      {кнопка}
    </div>"""
    return _send(to_email, "Новый сотрудник — AOCG AI Офис", html)
...
def send_fns_data_email(to_email: str, продавец: str, ссылка: str = "") -> bool:
    ...
    html = f"""<div style="font-family:Arial,sans-serif;color:#111318">
      ...
      <p>Чек <b>{тело(продавец)}</b> заполнен полностью — проверять руками не нужно.</p>
      {кнопка}
    </div>"""
    return _send(
        to_email, тема(f"Чек дополнен данными ФНС: {продавец} — AOCG AI Офис"), html
    )
...
def send_invite_notification(
    ...
    html = f"""<div style="font-family:Arial,sans-serif;color:#111318">
      <h2 style="color:#A4161A">Приглашение в «{тело(org_name)}»</h2>
      <p>Вас пригласили в AOCG AI Офис (роль: {тело(role)}). Перейдите по ссылке, чтобы присоединиться:</p>
      <p><a href="{invite_url}">{invite_url}</a></p>
    </div>"""
    return _send(to_email, тема(f"Приглашение в «{org_name}» — AOCG AI Офис"), html)
