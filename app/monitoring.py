...
def init_sentry() -> bool:
    ...
    sentry_sdk.init(
        dsn=dsn,
        # ⚠️ ВТОРОЙ КАНАЛ УТЕЧКИ, ЗАКРЫТ 19.08.2026 (S-66). По умолчанию SDK
        # кладёт в КАЖДЫЙ кадр стека локальные переменные, и проба с маркером
        # нашла текст сообщения ШЕСТЬ раз: vars.body.text, vars.body_bytes,
        # vars.json_body.text, vars.values.req, vars.payload.text, vars.req.
        # `request.data` при этом был уже чист — то есть разрешительный список
        # закрыл один путь из двух.
        #
        # ПОЧЕМУ ВЫКЛЮЧЕНИЕ, А НЕ ФИЛЬТРАЦИЯ КАДРОВ: vars кадра — локальные
        # переменные ЧУЖОГО кода (starlette, fastapi). Наш разрешительный
        # список собран по НАШИМ моделям, и совпадение имени там было бы
        # случайностью, а не гарантией. Тот же довод, по которому мы не
        # спускаемся внутрь неразрешённого узла в _scrub_request_data.
        #
        # ЦЕНА ИЗМЕРЕНА И ОНА ПОКА НУЛЕВАЯ: за всё время работы приёмника
        # (с 22.06.2026) в нём 8 событий, все восемь — шлюз MAX. Ни один
        # разбор на значения переменных не опирался. ⚠️ Но «не пользовались»
        # не значит «не понадобится»: 500 у нас редки, и первая настоящая
        # авария — первый случай, когда переменные могли бы пригодиться.
        #
        # ЭТО ОБРАТИМО ОДНОЙ СТРОКОЙ, И В ЭТОМ СМЫСЛ ВЫБОРА: в момент разбора
        # настоящей аварии значение временно ставится в True, причина ищется,
        # флаг возвращается. У фильтрации кадров такой ручки нет — там
        # поверхность постоянная. Возмещение диагностики — T56: признак
        # «что не так» пишется в НАШ лог, а не отправляется наружу.
