...
def _свод_объединением_источников(g) -> Optional[dict]:
    """Свод НДС: ОБЪЕДИНЯЮТСЯ ИСТОЧНИКИ, СТАВКИ ОСТАЮТСЯ РАЗДЕЛЬНЫМИ.

    ⚠️ ИМЯ ВЫБРАНО ПРОТИВ ОДНОГО КОНКРЕТНОГО ПРОЧТЕНИЯ, требование владельца
    28.08.2026. Слово «слияние» читается как объединение СТАВОК — то есть
    ровно наоборот тому, что здесь происходит, и как раз то, что задача №28
    запрещает: 20 и 20/120 не сливаются, 5 и 6 не сливаются, каждая из
    двенадцати ставки тега 1199 остаётся отдельной строкой свода.
    Объединяются ИСТОЧНИКИ данных, а не ставки.

    Для каждой ставки берётся первый источник, где она ЕСТЬ. Суммы разных
    источников НЕ складываются — иначе ставка, присутствующая и в контейнере,
    и в позициях, удвоилась бы.
    """
    итог: dict = {}
    for источник in (
        _breakdown_из_контейнера(g),
        _vat_breakdown(g("items")),
        _breakdown_из_верхних(g),
    ):
        for ставка, сумма in (источник or {}).items():
            итог.setdefault(ставка, сумма)
    return итог or None
...
def parse_fns_response(raw_data: dict) -> dict:
    ...
    return {
        ...
        # ═══ КАНОН СВОДА НДС: ТРИ ИСТОЧНИКА, ОБЪЕДИНЕНИЕ ИСТОЧНИКОВ (№28 ②) ═══
        #
        # ① КОНТЕЙНЕР 1115 — свод самой ФНС.
        # ② ПОЗИЦИИ (тег 1199 + ndsSum) — НАШ расчёт, но по данным чека.
        # ③ ПЛОСКИЕ ТЕГИ 1102–1107 — старое поколение, включая расчётные.
        #
        # ⚠️ ПРИОРИТЕТ ПРИМЕНЯЕТСЯ К КАЖДОЙ СТАВКЕ ОТДЕЛЬНО, А НЕ К ИСТОЧНИКУ
        # ЦЕЛИКОМ. Для ставки берётся первый источник, где она ЕСТЬ; суммы
        # разных источников НЕ складываются, поэтому двойного счёта не будет.
        #
        # ⚠️ ПЕРВАЯ РЕДАКЦИЯ (87c006b) БРАЛА ИСТОЧНИК ЦЕЛИКОМ — `A or B or C` —
        # И ТЕРЯЛА ДЕНЬГИ. Посылка была: «контейнер 1115 — полный свод ФНС».
        # ЗАМЕР 28.08.2026 НА 67 ЧЕКАХ ЕЁ ОПРОВЕРГ: контейнер ЧАСТИЧЕН, он
        # несёт только те ставки, у которых НЕТ плоского тега. Чек id=24:
        # контейнер [{nds:11, ndsSum:35469}] — одна ставка 22%; при этом
        # nds10=10046 лежит плоским тегом, а позиции несут ОБЕ ставки с
        # суммами. Приоритет целиком выбрасывал строку 10%.
        # **У ОДИННАДЦАТИ ЧЕКОВ (24, 31, 43, 46, 55, 58, 62, 75, 82, 86, 93)
        # это стоило бы 1 956,23 ₽ входящего НДС.** Поймано сухим прогоном
        # пересчёта, на стоп-проверке «контейнер против позиций», которую
        # владелец потребовал поставить ДО начала работ.
        #
        # ⚠️ ПОЧЕМУ КОНТЕЙНЕР ВСЁ ЖЕ ПЕРВЫЙ ТАМ, ГДЕ СТАВКА ЕСТЬ У ОБОИХ —
        # довод владельца, принят дословно: касса и ФНС согласились между
        # собой, наша арифметика по позициям третье мнение и слабее двух
        # первых. Замер это подтверждает: на общих ставках все источники
        # сходятся ДО КОПЕЙКИ во всех 67 чеках, спора о величине нет вовсе —
        # есть только ОТСУТСТВИЕ ставки в одном из источников.
        "vat_breakdown": _свод_объединением_источников(g),
