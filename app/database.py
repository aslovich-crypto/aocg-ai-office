...
async def init_db():
    ...
    async with p.acquire() as conn:
        await conn.execute("""
            ...
            -- ⚠️ ИНДЕКС ЧАСТИЧНЫЙ, И ДОВОД ПЕРЕПИСАН 08.09.2026 — прежний
            -- ПРОТУХ. Он ссылался на `UserCreate.email: str = ""`, а класс
            -- снесён 31.08.2026 вместе с ручкой POST /api/users/ (`7d74bb7`,
        ...
        # ── Ключи интеграции: доступ к API не от человека, а от машины ──
        #
        # ⚠️ ЗАЧЕМ ОТДЕЛЬНАЯ СУЩНОСТЬ, А НЕ ЛИЧНЫЙ ТОКЕН. Ключ уезжает НА ЧУЖУЮ
        # МАШИНУ — в базу клиента, в настройки обработки 1С, — и живёт там долго.
        # Личный токен человека туда отдавать нельзя: он открывает всё, что может
        # человек, и гасится только вместе с ним. До этой таблицы понятия
        # «принципал, который не пользователь» в приложении не было НИ ОДНОГО
        # (замер аудита 12.09.2026: 43 файла, 10 261 строка, ноль совпадений).
        #
        # ⚠️ ХРАНИТСЯ ХЕШ, А НЕ КЛЮЧ (решение владельца 12.09.2026). Утечка базы
        # не должна давать доступ. Тот же приём, что у `revoked_tokens`, где
        # хранится sha256 токена, и по той же причине.
        #
        # ⚠️ ПРЕФИКС — ОТКРЫТАЯ ЧАСТЬ, И ОН НУЖЕН ПО СУЩЕСТВУ, А НЕ ДЛЯ КРАСОТЫ:
        # по хешу строку не найти, не перебрав все ключи организации. Префикс
        # уникален глобально, по нему идёт поиск при проверке, он же видно
        # в списке и в журнале — секрет туда не попадает никогда.
        #
        # ⚠️ ДВУХ ЖИВЫХ КЛЮЧЕЙ СХЕМА НЕ ВЫРАЖАЕТ, И ЭТО НАМЕРЕННО. Правило
        # «ровно два живых на организацию» нужно для ПОВОРОТА ключа без простоя
        # (в базе клиента ключ меняют руками, и отзыв старого до вставки нового
        # останавливает интеграцию до визита). Частичный уникальный индекс запер
        # бы ровно ОДИН живой ключ, то есть выразил бы не то правило. Считается
        # в коде выпуска — заход ⑧.
        #
        # ⚠️ ВНЕШНИЙ КЛЮЧ НА `organizations` ЕСТЬ, в отличие от старых receipts
        # и reports, где `org_id` без ключа. Повторять тот долг в новой таблице
        # незачем: правило для новых таблиц записано в своде.
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS integration_keys (
                ...
                -- Последнее использование — КОЛОНКИ заводятся здесь, а пишет
                -- в них код захода ⑦. Без них на вопрос «пользуется ли ключом
                -- не тот» ответить нечем: журнала обращений в приложении нет
                -- вовсе (замер аудита 12.09.2026).
                last_used_at TIMESTAMPTZ,
                last_used_ip VARCHAR(45),
                use_count    BIGINT NOT NULL DEFAULT 0
            );
            -- Поиск при проверке идёт ПО ПРЕФИКСУ, и он обязан быть однозначным
            -- глобально, а не внутри организации: организацию мы узнаём ИЗ найденной
            -- строки, то есть на момент поиска её ещё не знаем.
            CREATE UNIQUE INDEX IF NOT EXISTS integration_keys_prefix_unique
                ON integration_keys(prefix);
            -- Список и проверка всегда спрашивают ЖИВЫЕ ключи; отозванные копятся
            -- и в выборку не попадают, поэтому индекс частичный.
        ...
        # у отчёта может быть сколько угодно, успешная — одна.
        # ⚠️ ГОНКУ ОН НЕ ЛОВИЛ, И ЭТО ЗАМЕР, А НЕ ДОГАДКА (1C-29, 22.09.2026).
        # Прежняя фраза здесь обещала обратное. Два одновременных нажатия
        # создавали в подделке 1С ДВА документа: индекс видит только `ok`,
        # а при боевой политике «никогда не проводить» исход `unposted`,
        # и индекс не участвует вовсе. Гонку держит второй индекс ниже —
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS odata_exports (
                ...
                error_note   TEXT,
                -- ⚠️ КАКИЕ КАТЕГОРИИ УЕХАЛИ НА СЧЁТ ПО УМОЛЧАНИЮ, А НЕ ПО
                -- ПРАВИЛУ (требование владельца 16.09.2026). Пустой массив
                -- значит «правила нашлись для всех», и это РАЗНЫЕ вещи
                -- с «не знаем»: поэтому NOT NULL с умолчанием, а не NULL.
                --
                -- ⚠️ ПОЧЕМУ ПЕРЕЧЕНЬ, А НЕ ФЛАГ. Требование «ответ обязан
                -- сказать, что подставлено по умолчанию» ШИРЕ МОМЕНТА ОТВЕТА:
                -- через месяц бухгалтер спросит, почему всё легло на 20.01,
                -- и ответ должен лежать В ЖУРНАЛЕ, а не в памяти. Флаг скажет
                -- «подстановки были», перечень — КАКИЕ КАТЕГОРИИ НАСТРОИТЬ.
                --
                -- ⚠️ ИМЕНА, А НЕ id, И ЭТО НАМЕРЕННО. Журнал фиксирует момент:
                -- если категорию потом переименуют, здесь останется то имя,
                -- которое видел человек в день выгрузки. Для отчёта «почему
                -- так легло» это вернее нынешнего имени.
            ...
            -- ⚠️ ЖИВАЯ ВЫГРУЗКА У ОТЧЁТА ОДНА: ИДУЩАЯ ИЛИ ЗАКОНЧЕННАЯ С ДОКУМЕНТОМ
            -- (1C-29, защита от двойного нажатия, часть 1C-23). Идущая
            -- (`running`) входит в набор намеренно: второй одновременный
            -- запрос спотыкается о ПЕРВУЮ ЖЕ запись журнала, ДО обращения
            -- к 1С, и получает 409. Проверка в коде этого не заменяет: два
            -- запроса читают журнал одновременно и оба видят его пустым.
            -- Отменённые и неудачные в набор не входят — они история.
            ...
            -- ⚠️ ОТДЕЛЬНЫМ ALTER, И ЭТО НЕ ИЗБЫТОЧНОСТЬ: `CREATE TABLE IF NOT
            -- EXISTS` на УЖЕ СОЗДАННОЙ таблице не добавляет колонок вовсе.
            -- Где таблица успела появиться прошлой редакцией DDL (тестовые
