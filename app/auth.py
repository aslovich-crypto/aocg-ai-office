...
# ⚠️ РОЛЬ КЛЮЧА ИНТЕГРАЦИИ ОБЪЯВЛЕНА ЗДЕСЬ И НАМЕРЕННО НЕ ВХОДИТ НИ В `ROLES`,
# НИ В `Role` (шаг 3, заход ④, решение владельца 12.09.2026). Это не описка
# и не «забыли дописать» — на этом держится вся безопасность принципала-ключа.
#
# ЧТО ДАЁТ НЕВХОЖДЕНИЕ В `ROLES`: по нему валидируется ВСЁ, что заводит
# ЧЕЛОВЕКА (приглашение, создание пользователя). Роли, которой там нет,
# человеку не назначить — то есть завести «сотрудника с правами ключа»
# нельзя даже руками через API.
#
# ЧТО ДАЁТ НЕВХОЖДЕНИЕ В `Role`: pydantic отвергает такое значение на входе,
# значит роль нельзя протащить телом запроса.
#
# ЧТО ДАЁТ НЕВХОЖДЕНИЕ В ОБА: `can_see_all` и `can_delete_any` сравнивают
# роль по РАВЕНСТВУ с именами людей, поэтому для ключа обе возвращают ложь
# ПО ПОСТРОЕНИЮ. Ключ не получает прав — не «теряет их задним числом»,
# а не получает изначально.
#
# ⚠️ И ПОЧЕМУ ЭТО НЕ ТОТ ДУБЛЬ, ПРОТИВ КОТОРОГО ЗАВЕДЕНО 13-Е НАБЛЮДЕНИЕ
# ГЛАВЫ 13а.18. Там речь про канон, объявленный ДВАЖДЫ и обязанный совпадать
# (кортеж `ROLES` и `Literal Role`). Здесь третье объявление НЕ ОБЯЗАНО
# входить в те два — оно обязано в них НЕ входить, и это требование
# противоположное. Оба требования проверяются сторожом
# `tests/test_integration_role_isolated.py`, а не памятью: и совпадение
# первых двух, и невхождение третьего.
