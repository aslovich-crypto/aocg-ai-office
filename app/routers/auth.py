...
async def invite_validate(token: str):
    ...
    return {
        ...
        # ⚠️ АДРЕС ОТДАЁМ ЦЕЛИКОМ, НЕ МАСКОЙ (решение владельца 05.09.2026).
        # Ссылка и есть секрет: кто её открыл, держит токен на вход в
        # организацию — маска защищала бы менее ценное, чем то, что уже отдано,
        # а человеку с двумя почтами создала бы тупик «подставьте угаданное».
        # Фронту нужны обе величины: признак решает, спрашивать ли адрес,
        # сам адрес — что подставить в поле (JoinScreen).
        "is_personal": bool(row and row["email"]),
...
async def register_by_invite(body: RegisterByInviteIn, background: BackgroundTasks):
    ...
    # ⚠️ ИМЕННАЯ ССЫЛКА СВЕРЯЕТ АДРЕС — И ЭТА ПРОВЕРКА СТОИТ ДО ПРОВЕРКИ
    # ЗАНЯТОСТИ ПОЧТЫ, А НЕ ПОСЛЕ. Порядок здесь — не стиль, а защита: после
    # неё чужой адрес сначала получал бы ответ «этот email уже зарегистрирован»,
    # то есть приглашение работало бы проверялкой чужих почт для любого, кому
    # ссылка попала в руки.
    #
    # Замер прода 04.09.2026: приглашение №8 звало на a.shu@aocg.ru, человек
    # ввёл тот же адрес — но совпал ЧЕЛОВЕК, а не система: `inv["email"]`
    # не читался здесь ни разу.
    #
    # Сверка нечувствительна к регистру и пробелам: адрес в базе приведён
    # к нижнему регистру при создании, введённый — строкой выше.
    # ⚠️ ОБЩАЯ ССЫЛКА (`email IS NULL`) СВЕРКУ НЕ ПРОХОДИТ: адресата у неё нет,
    # годится любая почта — это её природа, а не недосмотр.
    if inv["email"] and inv["email"].strip().lower() != email:
        # ⚠️ ВЕРНЫЙ АДРЕС В ОТКАЗЕ НЕ НАЗЫВАЕМ. Отказ уходит тому, кто ввёл
        # ЧУЖУЮ почту, и он не обязан узнавать, кого на самом деле звали.
        raise HTTPException(
            status_code=400,
            detail=(
                "Это приглашение выписано на другой адрес — войдите с той "
                "почты, на которую его прислали"
            ),
        )
