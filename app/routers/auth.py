...
def _public_user(u: dict) -> dict:
    keys = ("id", "first_name", "last_name", "patronymic", "email", "phone",
            "role", "org_id", "is_email_verified", "is_active")
...
async def _org(p, org_id) -> Optional[dict]:
    if not org_id:
        return None
    row = await p.fetchrow("SELECT id, name, inn, type FROM organizations WHERE id=$1", org_id)
...
async def verify_email(token: str):
    ...
    await p.execute("UPDATE users SET is_email_verified=true, email_verify_token=NULL WHERE id=$1", row["id"])
    return await _auth_payload(p, row)
...
async def login(request: Request, body: LoginIn):
    ...
    p = await get_pool()
    row = await p.fetchrow("SELECT * FROM users WHERE lower(email)=lower($1) OR phone=$1", ident)
    if not row:
    ...
    if u.get("locked_until") and u["locked_until"] > now:
        ...
        raise HTTPException(status_code=429, detail=f"Слишком много попыток. Попробуйте через {mins} мин")
    ...
    if not verify_password(body.password, u.get("password_hash")):
        ...
        await p.execute("UPDATE users SET failed_attempts=$1, locked_until=$2 WHERE id=$3",
                        attempts, locked, u["id"])
        if locked:
            raise HTTPException(status_code=429, detail="Слишком много попыток. Попробуйте через 15 мин")
        raise HTTPException(status_code=401, detail="Неверный логин или пароль")

    if not u.get("is_email_verified"):
        raise HTTPException(status_code=403, detail="Подтвердите email")

    await p.execute("UPDATE users SET failed_attempts=0, locked_until=NULL, last_login_at=NOW() WHERE id=$1", u["id"])
...
async def logout(body: LogoutIn):
    if body.refresh_token:
        ...
        await p.execute("INSERT INTO revoked_tokens (token_hash, expires_at) VALUES ($1,$2)", th, exp)
    return {"ok": True}
...
async def me(user: dict = Depends(get_current_user)):
    p = await get_pool()
    return {"user": _public_user(user), "organization": await _org(p, user.get("org_id"))}
    ...
    async with p.acquire() as conn:
        async with conn.transaction():
            user = await conn.fetchrow(
                ...
                body.first_name, body.last_name, email, body.phone,
                hash_password(body.password), inv["role"], inv["org_id"], auto_verify, verify_tok)
            new_uses = inv["uses_count"] + 1
            await conn.execute("UPDATE invite_links SET uses_count=$1, is_active=$2 WHERE id=$3",
                               new_uses, new_uses < inv["max_uses"], inv["id"])
...
async def invite_delete(token: str, user: dict = Depends(get_current_user)):
    _require_admin(user)
    p = await get_pool()
    await p.execute("UPDATE invite_links SET is_active=false WHERE token=$1 AND org_id=$2", token, user["org_id"])
    return {"ok": True}
...
async def egrul(inn: str):
    ...
    try:
        async with httpx.AsyncClient(timeout=8.0, headers={"User-Agent": "Mozilla/5.0"}) as client:
            init = await client.post("https://egrul.nalog.ru/", data={"query": digits, "page": ""})
            tok = init.json().get("t")
            if not tok:
                return None
            for _ in range(5):
                ...
                if rows:
                    item = rows[0]
                    return {"name": item.get("c") or item.get("n"),
