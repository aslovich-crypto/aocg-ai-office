...
class ReceiptIn(BaseModel):
    ...
    # ⚠️ ВРЕМЯ ЧЕКА ПРИНИМАЕТСЯ ОТ КЛИЕНТА С 12.09.2026 (строка 31, правка ⑤),
    # и без него четыре предыдущие правки не дают результата. Разбор:
    # `_можно_дозапросить` требует ПЯТЬ значений — ФН, ФД, ФПД, время и сумму,
    # потому что запрос к ФНС это строка QR, а в ней `t=` — дата и время
    # ДО МИНУТЫ. Время писалось ТОЛЬКО из разобранного `raw_data`, то есть
    # только когда ФНС уже ответила, — а нужно оно ровно в обратном случае.
    # Значит чек, заведённый по QR при молчащей ФНС, оставался без права
    # на дозапрос НАВСЕГДА даже с тремя сохранёнными реквизитами.
    # ⚠️ ЗОНЫ ЗДЕСЬ НЕТ И БЫТЬ НЕ ДОЛЖНО: это СТЕННОЕ время кассы, часовой
    # пояс ККТ не присылает ни ФНС, ни QR. Колонка с 11.09.2026 —
    # `TIMESTAMP WITHOUT TIME ZONE` ровно поэтому (строка 20).
...
async def create_receipt(r: ReceiptIn, user: dict = Depends(get_current_user)):
    ...
    # Было: `and source != "photo_ocr"` — ФН объявлялся ненадёжным по ЯРЛЫКУ
    # ИСТОЧНИКА. Ярлык описывает ПОСЛЕДНИЙ шаг, а не происхождение каждого
    # поля: путь «QR отсканирован → ФНС промолчала → распознали фото» кончается
    # ярлыком photo_ocr, хотя ФН там машинно прочитан с QR, а не угадан.
    # ⚠️ ЧЕМ ДОКАЗАНО, ЧТО РАСПОЗНАННЫЙ ФН СЮДА НЕ ПОПАДЁТ: промпт
    # распознавания ПРЯМО запрещает читать ФН (`app/routers/ocr.py`, правило
    # «НЕ извлекай ФН, ЗН ККТ и РН ККТ»), а разбор ответа всегда возвращает
    # по нему None — это закреплено тестом `test_parse_ocr_response_kkt_fn_always_none`,
    # который подсовывает разбору выдуманный ФН и требует None. Значит ФН
    # в теле запроса может прийти ТОЛЬКО с QR или с бумаги от человека.
    # Вариант A остаётся в силе для СВОЕГО случая: цифре, ПРОЧИТАННОЙ МОДЕЛЬЮ,
    # в реквизитах по-прежнему не место — её туда просто неоткуда взять.
    has_reliable_fn = bool(effective_kkt_fn)
    ...
    # ⚠️ ХРАНИМОЕ ЗНАЧЕНИЕ И ОСНОВАНИЕ ДЕДУПА — ОДНО И ТО ЖЕ, И ЭТО ИНВАРИАНТ
    # (ЧП C). Писать ФН, по которому дубль не искали, значит сделать решение
    # дедупа невоспроизводимым задним числом. Поэтому ярлык источника снят
    # в ОБОИХ местах разом, а не только здесь: разведи их — и колонка снова
    # начнёт расходиться с тем, по чему шла проверка.
        ...
        async with p.acquire() as conn:
            async with conn.transaction():
                ...
                row = await conn.fetchrow(
                    ...
                    source,
                    r.photo_url,
                    # ⚠️ ОТВЕТ ФНС В ПРИОРИТЕТЕ, КЛИЕНТ — ЗАПАСНОЙ ПУТЬ. Тот же
                    # порядок, что у ФД и ФПД: разобранное значение точнее
                    # набранного, но при молчащей ФНС берём что есть.
                    parsed.get("datetime") or r.datetime,
                    parsed.get("currency"),
