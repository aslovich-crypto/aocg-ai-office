...
# ⚠️ ВХОД В ТРЕКЕР (T199 ①): новая строка — только со штампом решения владельца,
# номер из истории повторно не выдаётся. Красное — в CI, здесь ручной прогон.
## tracker-entry — вход в трекер: строки без решения владельца, повторные номера
tracker-entry:
	@$(PY) tests/tools/tracker_entry_guard.py

## tracker-entry-mut — мутации сторожа входа (на копиях, рабочие файлы не трогаются)
tracker-entry-mut:
	@$(PY) tests/tools/tracker_entry_mutations.py
...
## odata-export-mut — мутации выгрузки отчёта в 1С (ссылки, подстановки, повтор, отмена)
odata-export-mut:
	@$(PY) tests/tools/odata_export_mutations.py
